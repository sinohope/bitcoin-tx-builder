...
func TestSignTx(t *testing.T) {
	// legacy address
	{
		...
		// build unsigned tx
		{
			...
			var tx *wire.MsgTx
			var err error
			tx, prevTxOuts, err = txBuild.Build(false)
			assert.Nil(t, err)
			data, err = GetTxHex(tx)
			assert.Nil(t, err)
		}

		// sign
		{
			// send to sign service
			tx, err := NewTxFromHex(data)
			assert.Nil(t, err)

			privateKeyWif, err := btcutil.DecodeWIF("cPnvkvUYyHcSSS26iD1dkrJdV7k1RoUqJLhn3CYxpo398PdLVE22")
			assert.Nil(t, err)
			privateKey := privateKeyWif.PrivKey
			privateKeys := make([]*btcec.PrivateKey, 1)
			privateKeys[0] = privateKey

			prevOutFetcher := txscript.NewMultiPrevOutFetcher(nil)
			for i := 0; i < len(tx.TxIn); i++ {
				var txHash *chainhash.Hash
				txHash, err = chainhash.NewHashFromStr(tx.TxIn[i].PreviousOutPoint.Hash.String())
				assert.Nil(t, err)
				outPoint := wire.NewOutPoint(txHash, tx.TxIn[i].PreviousOutPoint.Index)
				prevOutFetcher.AddPrevOut(*outPoint, prevTxOuts[i])
			}

			err = Sign(tx, privateKeys, prevOutFetcher)
			assert.Nil(t, err)
